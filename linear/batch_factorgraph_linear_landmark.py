...
def batch_factorgraph_example():
    ...
    # Plot the marginals with mapping to manifold
    plot_result(pose_marginals, landmark_marginals)
    #result.print("\nFinal Result:")

    # Plot using gtsam's built-in plotting functions (marginalizes out orientation)
    fig = plt.figure(2)
    ...
    for key in poses.keys():
        ...
        gtsam_plot.plot_pose2_on_axes(axes, pose, covariance=covariance, axis_length=0.3)

    # Plot 2D landmarks
    landmarks: np.ndarray = gtsam.utilities.extractPoint2(result)  # 2xn array
    for j, landmark in enumerate(landmarks):
        gtsam_plot.plot_point2_on_axes(axes, landmark, linespec="b")
        covariance = marginals.marginalCovariance(L(j+1))
        gtsam_plot.plot_covariance_ellipse_2d(axes, landmark, covariance=covariance)
