...
def batch_factorgraph_example():
    ...
    X4 = X(4)
    X5 = X(5)
    pose_variables = [X1, X2, X3, X4, X5]

    # Create keys for the landmarks.
    L1 = L(1)
    L2 = L(2)
    L3 = L(3)
    landmark_variables = [L1, L2, L3]
    ...
    graph.add(gtsam.BetweenFactorPose2(
        X3, X4, gtsam.Pose2(2.0, 0.0, 0.0), odometry_noise))
    graph.add(gtsam.BetweenFactorPose2(
        X4, X5, gtsam.Pose2(2.0, 0.0, 0.0), odometry_noise))
    ...
    graph.add(gtsam.BearingRangeFactor2D(
        X4, L2, gtsam.Rot2.fromDegrees(135), np.sqrt(4.0 + 4.0), measurement_noise))
    graph.add(gtsam.BearingRangeFactor2D(
        X5, L3, gtsam.Rot2.fromDegrees(90), 2.0, measurement_noise))
    ...
    initial_estimate.insert(X4, gtsam.Pose2(6.20, 0.20, -0.10))
    initial_estimate.insert(X5, gtsam.Pose2(8.00, -0.10, 0.20))
    initial_estimate.insert(L1, gtsam.Point2(1.80, 2.10))
    initial_estimate.insert(L2, gtsam.Point2(4.10, 1.80))
    initial_estimate.insert(L3, gtsam.Point2(6.20, 0.20))
