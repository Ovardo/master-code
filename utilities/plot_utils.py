...
def plot_result(ax, poses, landmarks, poses_gt=None, gt_landmarks=None, sample_points=False, exact_map=True):
    ...
    for dist, color in zip(poses, pose_cmap([number / num_poses for number in range(num_poses)])):
        ...
        if exact_map : # Plot covariance from manifold (takes rotation into consideration)
            plot_se2_covariance_on_manifold_gtsam(ax, dist, fill_alpha=0.1, fill_color="r")
        else: # Plot covariance of translation (marginalizes out rotation, i.e disregard it).
            dist = MultivariateNormalParameters(
                mean=mean_translation,
                covariance=dist.covariance[0:2, 0:2]
            ) 
            plot_ellipse(ax, dist, fill_alpha=0.1, fill_color="r") # 5.99146454711 approx 95% for 2D
        ...
        if sample_points:
            if isinstance(dist.mean, gtsam.Pose2):
                ...
                print("Warning: Sampling points for pose marginals only implemented for gtsam.Pose2 means.")
            
        # Plot mean.
        ax.plot(mean_translation[0], mean_translation[1], marker='o', color='r', linestyle='')
    ...
    if gt_landmarks is not None:
        ax.plot(gt_landmarks[0, :], gt_landmarks[1, :], marker='x', color='b', linestyle='', markersize=8)
    ...
    if poses_gt is not None and len(poses_gt) > 0:
        ...
        ax.plot(last_translation[0], last_translation[1], marker='x', color='g', linestyle='', markersize=10)
...
def plot_se2_covariance_on_manifold_gtsam(ax, dist,
                                          n=50, chi2_val=7.815, right_perturbation=True, #99: 11.345, 95: 7.815, 
                                          fill_alpha=0., fill_color='lightsteelblue', linestyle='-',
                                          axis_length: float = 0.5, axis_color: str = 'k'):
...
def plot_pose2_on_axes(ax, pose,
                      axis_length: float = 0.25,
                      show_axis: bool = False, linestyle: str = '-', color: str = 'black', marker = ''):
  
    
    ax.plot(pose.x(), pose.y(), marker=marker, linestyle=linestyle, color=color, markersize=6)
    ...
    if show_axis:
        try:
            if isinstance(pose, gtsam.Pose2):
                ...
                ax.plot([cx, x_end[0]], [cy, x_end[1]], color='r', linewidth=0.8, linestyle='-')
                ax.plot([cx, y_end[0]], [cy, y_end[1]], color='g', linewidth=0.8, linestyle='-')
            else:
...
def plot_pose2_trajectory(ax, poses,
                           color='black', linestyle='-', linewidth=1.0,
                           show_axes=False, axis_length=0.25):
    ...
    if show_axes:
        ...
        ax.plot([x, x + axis_length * c],
                [y, y + axis_length * s], color='r', linewidth=0.8)
        ax.plot([x, x - axis_length * s],
                [y, y + axis_length * c], color='g', linewidth=0.8)
...
def plot_ellipse(ax, dist, n=50, chi2_val=5.991, fill_alpha=0., fill_color='lightsteelblue', linestyle='-', linewidth=1.0):
    u, s, _ = np.linalg.svd(dist.covariance)
    scale = np.sqrt(chi2_val * s)
